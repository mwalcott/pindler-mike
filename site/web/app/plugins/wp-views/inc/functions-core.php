...
function wpv_layout_users_V( $menu ) {
    ...
    foreach ( $user_shortcodes as $shortcode_slug => $shortcode_data ) {
		$menu[$basic][$shortcode_data['label']] = array( 
			...
			$shortcode_data['code'], 
			$basic, 
			"WPViews.shortcodes_gui.wpv_insert_popup('wpv-user', '" . esc_js( $shortcode_data['label'] ) . "', {field:{value:'" . esc_js( $shortcode_slug ) . "',label:'". esc_js( $shortcode_data['label'] ) . "'}}, '" . $nonce . "', this )"
		);
    }
