	...
	if ( in_array( 'user', $types ) ) {
		...
		foreach ( $user_shortcodes as $u_shortcode_slug => $u_shortcode_data ) {
			$editor->add_insert_shortcode_menu(
				...
				"WPViews.shortcodes_gui.wpv_insert_popup('wpv-user', '" . esc_js( $u_shortcode_data['label'] ) . "', {field:{value:'" . esc_js( $u_shortcode_slug ) . "',label:'". esc_js( $u_shortcode_data['label'] ) . "'}}, '" . $nonce . "', this )"
			);
			$index += 1;
		}
	}
				...
				if ( $WP_Views->is_archive_view( $view->ID ) ) {
					...
					if ( 
						...
						$editor->add_insert_shortcode_menu(
							...
							'WPViews.shortcodes_gui.wpv_insert_view_shortcode_dialog(\'' . esc_js( $view->ID ) . '\', \'' . esc_js( $view->post_title ) . '\', \'' . esc_js( $view->post_name ) . '\', \'' . esc_js( $current_id ) . '\', \'' . $nonce . '\')'
						);
						$index += 1;
					}
					if ( 
...
function wpv_post_taxonomies_editor_addon_menus_wpv_views_filter($items) {
	...
	foreach ($taxonomies as $taxonomy_slug => $taxonomy) {
		...
		$add[__('Taxonomy', 'wpv-views')][$taxonomy->label] = array(
			...
			"WPViews.shortcodes_gui.wpv_insert_popup('wpv-post-taxonomy', '" . esc_js( $taxonomy->label ) . "', {type:{value:'" . esc_js( $taxonomy_slug ) . "',label:'". esc_js( $taxonomy->label ) . "'}}, '" . $nonce . "', this )"
		);
	}
