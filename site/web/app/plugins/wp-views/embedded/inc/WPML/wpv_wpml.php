 ...
 *
 * @since 1.5.0
 * @since 2.2.2 Deprecated, not used anywhere in the Views codebase. Keep for backwards compatibility.
 */
function wpv_parse_wpml_shortcode( $content ) {
	
	_doing_it_wrong(
		'wpv_parse_wpml_shortcode', 
		__( 'This function will get deprecated in Views 2.3.0.', 'wpv-views' ),
		'2.2.2'
	);
...
function wpv_force_shortcodes_gui_wpml_string( $menu = array() ) {
	...
	if (
		...
		$menu[$basic][$wpml_string_title] = array( 
			...
			'wpml-string',
			$basic,
			"WPViews.shortcodes_gui.wpv_insert_popup('wpml-string', '" . esc_js( $wpml_string_title ) . "', {}, '" . $nonce . "', this )"
		);
	}
	return $menu;
}
	...
	public function wpv_add_wpml_shortcodes_to_editor( $editor, $nonce ) {
		if ( $this->is_wpml_loaded() ) {
			$editor->add_insert_shortcode_menu(
				...
				'wpml-lang-switcher',
				'WPML',
				"WPViews.shortcodes_gui.wpv_insert_popup('wpml-lang-switcher', '" . esc_js( __( 'Language selector', 'wpv-views' ) ) . "', {}, '" . $nonce . "', this )"
			);
			$editor->add_insert_shortcode_menu(
				__( 'Footer language selector', 'wpv-views' ),
				'wpml-lang-footer',
				'WPML',
				"WPViews.shortcodes_gui.wpv_insert_popup('wpml-lang-footer', '" . esc_js( __( 'Footer language selector', 'wpv-views' ) ) . "', {}, '" . $nonce . "', this )"
			);
			...
			if ( isset( $iclCMSNavigation ) ) {
				...
				*/
				$editor->add_insert_shortcode_menu(
					...
					'wpml-sidebar',
					'WPML',
					"WPViews.shortcodes_gui.wpv_insert_popup('wpml-sidebar', '" . esc_js( __( 'Sidebar navigation', 'wpv-views' ) ) . "', {}, '" . $nonce . "', this )"
				);
			}
		}
	}
